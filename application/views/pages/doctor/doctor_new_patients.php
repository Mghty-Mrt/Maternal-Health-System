    ...
    <div class="card">
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover text-nowrap mb-2 align-middle text-center" id="drnewpatients">
                            ...
                            <tr id="tr">
                                ...
                                <td class="border-bottom-0">
                                    <p class="mb-0 fw-normal">
                                        <?= $perNewP->firstname ?> <?= $perNewP->middlename ?> <?= $perNewP->lastname ?>
                                    </p>
                                </td>
                                <td class="border-bottom-0">
                                ...
                                <td class="border-bottom-0">
                                    <a href="../doctor/itrcheckup?id=<?= $perNewP->prid ?>" class="btn btn-secondary fs-2 btn-sm btnCheck" id="checkUpLink"><i class="fs-3 ti ti-file-pencil"></i> Checkup</a>
                                    <?php if($perNewP->lpre_id == $perNewP->prid){ ?>
                                        <span class="btn bg-warning-subtle fs-2 text-warning btn-sm btnLab" style="display: none;" data-bs-toggle="" data-bs-target=""><i class="fs-3 fw-bolder ti ti-circle-check"></i>  Request Laboratory</span>
                                    <?php } else { ?>
                                        <span class="btn btn-warning fs-2 btn-sm btnLab" style="display: none;" data-bs-toggle="modal" data-bs-target="#dynamicmodalLab"><i class="fs-3 ti ti-zoom-question" data-id="<?= $perNewP->prid ?>"></i> Request Laboratory</span>
                                    <?php } ?>
                                </td>
                                <td>
                                    <div class="dropdown dropstart">
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton" data-popper-placement="left-start">
                                            <li>
                                                <a class="dropdown-item d-flex align-items-center gap-3 text-secondary" href="../doctor/editnewp?id=<?php print $perNewP->prid ?>"><i class="fs-4 ti ti-eye"></i>View</a>
                                            </li>
                                            <!-- <li>
...
<script>
    $(document).on('click', '.btnLab', function() {
        var pre_patient_id = $(this).find('i').attr('data-id');
        //alert(pre_patient_id);

        $.ajax({
            url: '../doctor/rqstLab?=' + pre_patient_id,
            method: 'GET',
            data: {
                pre_patient_id: pre_patient_id,
            },
            success: function(data) {
                $('#modalTitle').html('<div class="text-light"> <b> <i class="fas fa-file-prescription"></i> Request Laboratory</b> </div>');
...
<script>
    ...
    document.getElementById('checkUpLink').addEventListener('click', function(event) {
        event.preventDefault();

        showLoadingModal();

        var checkUpUrl = this.getAttribute('href');
