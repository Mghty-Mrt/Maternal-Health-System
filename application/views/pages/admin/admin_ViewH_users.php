<form>
    <?php foreach($hUserData as $perHuser) { ?>
    <div class="row">
        <div class="col-md-8">
        <?php if($perHuser->image == "") { ?>
            <img src="../assets/images/profile/default.jpg" class="rounded-circle float-start me-3" height="150" alt="profile"/>
            <?php } else { ?>
            <img src="/mhs_micro/profile/<?= $perHuser->image ?>" class="rounded-circle float-start me-2" height="150" width="155" alt="profile"/>
            <?php } ?>
            <p class="card-title mt-4 fw-semibold"><?= $perHuser->lastname ?>, <?= $perHuser->firstname ?> <?= $perHuser->middlename ?> <br><small class="fs-3"><?= $perHuser->rname ?></small></p>
            <p class="fs-3"><i class="ti ti-mail"></i> <?= $perHuser->email ?> <br><span class="fs-3"><i class="ti ti-phone"></i> <?= $perHuser->mobileno ?> </span></p>
        </div>
