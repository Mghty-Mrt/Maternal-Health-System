<form>
    <?php foreach($hcUserData as $perHCuser) { ?>
    <div class="row">
        <div class="col-md-8">
          <?php if($perHCuser->image == "") { ?>
            <img src="../assets/images/profile/default.jpg" class="rounded-circle float-start me-3" height="150" alt="profile"/>
            <?php } else { ?>
            <img src="/mhs_micro/profile/<?= $perHCuser->image ?>" class="rounded-circle float-start me-2" height="150" width="155" alt="profile"/>
            <?php } ?>
            <p class="card-title mt-4 fw-semibold"><?= $perHCuser->lastname ?>, <?= $perHCuser->firstname ?> <?= $perHCuser->middlename ?> <br><small class="fs-3"><?= $perHCuser->rname ?></small></p>
            <p class="fs-3"><i class="ti ti-mail"></i> <?= $perHCuser->email ?> <br><span class="fs-3"><i class="ti ti-phone"></i> <?= $perHCuser->mobileno ?> </span></p>
        </div>
